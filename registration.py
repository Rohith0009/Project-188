...
firebase = firebase.FirebaseApplication(
    "https://stupid-f855c-default-rtdb.firebaseio.com/", None
)
...
def login():
    ...
    hexadecimalPss = encryptedPass.hexdigest()
    get_password = firebase.get("/", username)

    if get_password != None:
        if get_password == hexadecimalPss:
...
def register():
    ...
    encryptedPass = hashlib.md5(password.encode())
    hexadecimalPss = encryptedPass.hexdigest()
    print(hexadecimalPss)
    put_data = firebase.put("/", username, hexadecimalPss)
...
def login_window():
    ...
    btn_login = Button(
        login_window, text="Log In", font="arial 13 bold", command=login, relief=FLAT
    )
